...
(fn colorize-comment [keywords colors l]
  (set comment-match (and keywords.comment-pattern
                          (string.find l keywords.comment-pattern)))
  (if (= comment-match 1)
      [colors.comment l]
      comment-match
...
(fn colorize [keywords colors lines abc]
  (lume.map lines (partial colorize-string keywords colors)))
