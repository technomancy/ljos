...
(fn close [confirm]
  ...
  (if (and state.b.needs-save? (not confirm) (not state.b.props.no-file))
...
(fn save []
  ...
  (let [b (if (= state.b.path "minibuffer") (editor.internal.behind) state.b)]
    (when (and b.needs-save? (not b.props.no-file))
      (state.fs.write b.path (.. (table.concat b.lines "\n") "\n"))
      (set b.needs-save? false))))
...
(fn history-prev []
  ...
  (when (< state.b.input-history-pos (# state.b.input-history))
    (set state.b.input-history-pos (+ state.b.input-history-pos 1))
    (let [n (+ (- (# state.b.input-history) state.b.input-history-pos) 1)]
      (replace-input (or (. state.b.input-history n) "")))))

(fn history-next []
  "Cycle the current input forward thru input history."
  (if (>= state.b.input-history-pos 0)
      (do (set state.b.input-history-pos (- state.b.input-history-pos 1))
          (let [n (+ (- (# state.b.input-history) state.b.input-history-pos) 1)]
            (replace-input (or (. state.b.input-history n) ""))))
      (do (set state.b.input-history-pos 0)
          (replace-input ""))))
...
If the mode doesn't provide an indenter, indent to match the previous line."
  ...
  (let [get-indentation (editor.get-prop :indentation default-indent)
        indentation (get-indentation state.b.lines (- state.b.point-line 1))
...
(fn scroll [n]
  (set state.b.point-line (-> (+ state.b.point-line (* (window-lines) (or n 1)))
                              (math.max 1)
...
(fn undo []
  (let [prev (. state.b.history (- (# state.b.history) state.b.undo-at))]
    (when (< state.b.undo-at (# state.b.history))
      (set state.b.undo-at (+ state.b.undo-at 1)))
    (when prev
      (set (state.b.lines state.b.point state.b.point-line)
           (values prev.lines prev.point prev.point-line))
      (editor.internal.run-on-change))))

{: find-file : close : search : replace : switch-buffer : echo-prop
 : next-buffer
 :prev-buffer (partial next-buffer -1)
 : focus-next : split : save : reload : revert : complete : go-to-line : execute
 : toggle-fullscreen :scale frontend.scale : history-prev : history-next
 : indent :newline-and-indent #(do (editor.cmd.newline) (indent))
